...
def normalize(DTYPE_t[::1] x, int n):
    cdef np.ndarray[DTYPE_t, ndim=1] output = np.empty((n,), dtype=np.float32)
    cdef DTYPE_t[::1] output_view = output
    get_variance(x, output_view, n)
    return output

@cython.boundscheck(False)
@cython.wraparound(False)
@cython.cdivision(True)
cdef  get_variance(DTYPE_t[::1] x, DTYPE_t[::1] output, int n):
    cdef Py_ssize_t i,j
    cdef float mean = 0, total = 0, variance = 0
    ...
    total = 0
    for i in range(n):
        total += cpow((x[i] - mean), 2)

    variance = total/n
    std = csqrt(variance + 1e-8)
    with nogil:
        for i in prange(n, schedule='static'):
            output[i] = (x[i] - mean) / std


def relu(DTYPE_t[:, ::1] x):

    cdef DTYPE_t[:, ::1] output = np.empty(
            (x.shape[0], x.shape[1]),
            dtype=np.float32)
    inner_relu(x, output, x.shape[0], x.shape[1])
    return output

@cython.boundscheck(False)
@cython.wraparound(False)
cdef inner_relu(DTYPE_t[:, ::1] x, DTYPE_t[:, ::1] output, int number, int size):
    for i in range(number):
        for j in range(size):
            output[i,j] = max(x[i,j], 0)